...
echo "==> [2/4] simulator Zero JVM + runtime image"
...
mkdir -p build
if ! xcodebuild -project TinyHttpServer.xcodeproj -scheme TinyHttpServer \
  -sdk iphonesimulator -configuration Debug -derivedDataPath build \
  ARCHS=arm64 build > build/sim-build.log 2>&1; then
  echo "ERROR: xcodebuild failed (full log: build/sim-build.log)" >&2
  grep -E 'error:' build/sim-build.log | sort -u | head -20 >&2
  exit 1
fi
echo "    BUILD SUCCEEDED (log: build/sim-build.log)"
